...
const val FLOWER_PHONE = "phone"
const val FLOWER_ADDRESS = "address"
...
class AddFlowerActivity : AppCompatActivity() {
    ...
    private lateinit var addFlowerPhone: TextInputEditText
    private lateinit var addFlowerAddress: TextInputEditText
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        addFlowerPhone = findViewById(R.id.add_flower_phone)
        addFlowerAddress = findViewById(R.id.add_flower_address)
    ...
    private fun addFlower() {
        ...
        if (addFlowerName.text.isNullOrEmpty() || addFlowerDescription.text.isNullOrEmpty() || addFlowerPhone.text.isNullOrEmpty() || addFlowerAddress.text.isNullOrEmpty()) {
            setResult(Activity.RESULT_CANCELED, resultIntent)
        } else {
            val name = addFlowerName.text.toString()
            val description = addFlowerDescription.text.toString()
            val phone = addFlowerPhone.text.toString()
            val address = addFlowerAddress.text.toString()

            resultIntent.putExtra(FLOWER_NAME, name)
            resultIntent.putExtra(FLOWER_DESCRIPTION, description)
            resultIntent.putExtra(FLOWER_PHONE, phone)
            resultIntent.putExtra(FLOWER_ADDRESS, address)
